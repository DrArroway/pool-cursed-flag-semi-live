...
* **capture.js**: A Puppeteer-driven scraping script that wakes up, navigates to the live EarthCam feed, handles cookie/consent banners automatically, and snaps a widescreen capture of the pool.
...
* It projects a tailored, non-uniform trapezoidal mapping matrix right onto the coordinate boundaries of the pool. It samples the background environmental light to establish a baseline scene brightness, then slices the perspective into 50 distinct regional anchor zones—allowing every single star on the United Statesian design to claim its own personal shade of swamp.
...
* **Algae Isolation Filter:** A targeted color-isolation algorithm. When toggled, it aggressively boosts green channels (G > R & G > B) by 25% while crushing competing values, forcing the flag of the US to fully embrace the bloom. You can toggle this off to inspect the raw, unadulterated environmental data.
...
* **Calibration Panel:** Real-time UI adjustment sliders mapping bounding coordinates (X, Y, W, H) and perspective scale factors directly to the canvas loop for immediate visual feedback so Muricans and international onlookers alike can fine-tune the view.
...
* **The Hardcore Route:** Fork the repository, optimize the matrix, and host your own cron jobs if you are a terminally online citizen of the USA.
